...
function Storage = Parkinson_DBS_random_square(k,b,h,step,t_final)
% This function solve a system of differential equations for a model of
% Parkinson's disease with DBS (3rd case) by Runge Kutta 4th order
...
Y = [0.1; 0.1];                       % initial conditions of x and y
Storage = zeros(length(t),6);           % creation array to store the result
Storage(1,:) = [time Y(1) Y(2) 0 0 0];  % init first column of the result (initial conditions)
...
for i = 1:(length(t))
        ...
        T = 0.1+(2-0.1)*rand;   % to generate a random period between 0.1 and 2
        freq = 1/T;             % frequency calculation
  ...
  k4 = step*[Z(2)-((2*b*Z(1))-((2/pi)*k*atan(Z(1)/h))); -((b*b*Z(1))+square(2*pi*freq*(time+step)))];
    ...
    Storage(i+1,:) = [time Y(1) Y(2) 0 0 freq]; % updating y, using old y
    
end

%------------------------------------------------------------------------
%------------------------------------------------------------------------
%% x_dot and y_dot calculation using for loop
%------------------------------------------------------------------------
%------------------------------------------------------------------------
...
for i = 1:length((t))
    ...
    % Calculate y_dot
    y_dot = -b*b*Storage(i,2) + square(Storage(i,5)*2*pi*Storage(i,1));
