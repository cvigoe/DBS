function Storage = Parkinson_DBS_random_square_euler(k,b,h,step,t_final)
% This function solve a system of differential equations for a model of
% Parkinson's disease with DBS (case 3) by Euler's method
...
% to generate random period between 0.1 and 2
T = 0.1 + (-0.1+2)*rand;
...
% updating y
Storage(i,3) = Storage(i-1,3) + step*((-b*b*Storage(i-1,2))+(square(2*pi*freq)*Storage(i,1)));
...
for i = 1:length((t))
    ...
    % Calculate y_dot
    y_dot = -b*b*Storage(i,2) + square(2*pi*Storage(i,6)*Storage(i,1));
